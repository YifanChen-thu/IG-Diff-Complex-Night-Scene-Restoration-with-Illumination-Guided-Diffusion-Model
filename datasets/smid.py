...
class smid:
    ...
    def get_loaders(self, parse_patches=True, validation='smid'):
        ...
        #lol
            ...
            print("=> evaluating outdoor sid test set...")
            path = os.path.join(self.config.data.data_dir, 'long_sid2')

        elif validation == 'smid':
            print("=> evaluating smid test set...")
            # path = os.path.join(self.config.data.data_dir, 'SMID_Long_np')
            test_dir = []
            f = open('/home/ysd21/cyf20/code/LowLightDiffusion/configs/test_list.txt')
            lines = f.readlines()
        ...
        train_dataset = LOLDataset(self.config.data.data_dir,
                                       ...
                                       transforms=self.transforms,
                                       #   filelist='allweather.txt',
                                       parse_patches=parse_patches,test_dir=test_dir,test=False)

        val_dataset = LOLDataset(self.config.data.data_dir, n=self.config.training.patch_n,
                                        patch_size=self.config.data.image_size,
                                        transforms=self.transforms,
                                        # filelist=filename,
                                        parse_patches=parse_patches,test_dir=test_dir,test=True)
...
class LOLDataset(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, n, transforms, filelist=None, parse_patches=True,test_dir=None,test=False):
        ...
        subfolders_input = util.glob_file_list(os.path.join(self.dir,'SMID_LQ_np'))
        subfolders_gt = util.glob_file_list(os.path.join(self.dir,'SMID_Long_np'))
        ...
        for subfolder_input,subfolder_gt in zip(subfolders_input,subfolders_gt):
            ...
            if (subfolder_name in test_dir):
                ...
                lenth_input = len(img_paths_input_test)
                img_paths_gt_test.extend(img_paths_gt_test*(lenth_input-1))
                ...
                lenth_input = len(img_paths_input_train)
                img_paths_gt_train.extend(img_paths_gt_train*(lenth_input-1))
                
                # print(len(img_paths_input_train2))

                # print("len(img_paths_gt_train):{},len(img_paths_input_train2):{}".format(len(img_paths_gt_train),len(img_paths_input_train2)))
        ...
        # self.input_names =  os.listdir(os.path.join(dir,'SMID_LQ_np'))
        # self.gt_names =  os.listdir(os.path.join(dir,'SMID_Long_np'))
    ...
    def get_images(self, index):
        ...
        # input_img = PIL.Image.open(os.path.join(self.dir, 'SMID_LQ_np',input_name)) if self.dir else PIL.Image.open(input_name)
        ...
        #     gt_img = PIL.Image.open(os.path.join(self.dir, 'SMID_Long_np',gt_name)) if self.dir else PIL.Image.open(gt_name)
        ...
        #     gt_img = PIL.Image.open(os.path.join(self.dir, 'SMID_Long_np',gt_name)).convert('RGB') if self.dir else PIL.Image.open(gt_name).convert('RGB')
