...
class LOLv2_real:
    ...
    def get_loaders(self, parse_patches=True, validation='lolv2_real'):
        ...
        train_dataset = LOLDataset(os.path.join(self.config.data.data_dir,'Train'),
                                       n=self.config.training.patch_n,
                                       patch_size=self.config.data.image_size,
                                       transforms=self.transforms,
                                       #   filelist='allweather.txt',
        ...
        val_dataset = LOLDataset(os.path.join(self.config.data.data_dir, 'Test'), 
                                n=self.config.training.patch_n,
                                        patch_size=self.config.data.image_size,
                                        transforms=self.transforms,
                                        # filelist=filename,
...
class LOLDataset(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, n, transforms, filelist=None, parse_patches=True):
        ...
        self.input_names =  sorted(os.listdir(os.path.join(dir,'Low')))
        self.gt_names =  sorted(os.listdir(os.path.join(dir,'Normal')))
    ...
    def get_images(self, index):
        ...
        gt_img_id = re.split('/', gt_name)[-1][:-4]

        assert img_id[-5:] == gt_img_id[-5:], f'{img_id}, {gt_img_id}'
        ...
        gt_img = PIL.Image.open(os.path.join(self.dir, 'Normal', gt_name)) if self.dir else PIL.Image.open(gt_name)
        ...
        #     gt_img = PIL.Image.open(os.path.join(self.dir, 'Normal',gt_name)).convert('RGB') if self.dir else PIL.Image.open(gt_name).convert('RGB')
